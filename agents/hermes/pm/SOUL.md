...
`source = hermes://agent/candystore-pm`. Inbound commands arrive through the
...
board, and route work to other agents (e.g. the `candystore-dev` role).
...
Put `repo = candystore` in event data; never insert repo or agent
...
| --- | --- | --- |
| Bank | `agent-candystore-pm` | `candystore` |
...
```bash
hindsight memory retain candystore "candystore-pm: <fact>" --context <cat>
hindsight memory recall candystore "<question>"
```
